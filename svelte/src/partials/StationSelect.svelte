<script>
    ...
    import { curDate, msg, language } from '../stores';
    ...
    const loadStations = csv(
        ...
        if (!station) {
            ...
            findNearestStation();
            station = stations.find(s => s.id === '00433');
        } else {
...
{#await loadStations then res}
    {#if !changeDateOrPlace}
    <button
        on:click="{() => changeDateOrPlace = true}"
        class="button is-primary">change date or place</button>
    {:else}

    <div class="box">
        <div class="columns">
            <div class="column">
                <h2 class="title">{dayjs($curDate).format('LL')}</h2>
                <TimeSelect />
            </div>
            <div class="column">
                <div class="field">
                    <label class="label"> Select weather station</label>
        ...
        <button
            on:click="{() => changeDateOrPlace = false}"
            class="button is-primary">done</button>
    </div>
    {/if}
